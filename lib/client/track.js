...
_SleepMode.prototype._startTimer = function() {
  ...
  if(!this.timer){
    this.timer = setInterval(function(){
      ...
      var condition;
      if(self.config.ddp){
        condition = lastTimeoutTime - self.lastDDPSendTime >= self.config.timeout;
      }
      if(self.config.focus){
        condition = condition || !document.hasFocus();
      }
      if(self.config.route){
        condition = condition || lastTimeoutTime - self.lastRouteTime >= self.config.timeout;
      }
      if(condition){
        self.sleepModeStatus = true;
        self.goSleepMode();
        self.emit('sleep', self);
      }
    }, this.config.timeout);
  }
...
_SleepMode.prototype._trackRouteChange = function() {
  var self = this;
  if(Router){
    Router.before(function(){
      self.lastRouteTime = new Date().getTime();
    });  
  }
  this._trackRouteChange = function(){};
};

_SleepMode.prototype._trackMouseFocus = function() {
  //document.hasFocus();
};
